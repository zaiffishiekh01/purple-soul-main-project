...
BEGIN
  ...
  -- Get admin ID
  SELECT id INTO admin_finance_id FROM admin_users WHERE role = 'finance' OR email LIKE '%finance%' LIMIT 1;

  -- If no finance admin, use any admin
  IF admin_finance_id IS NULL THEN
    SELECT id INTO admin_finance_id FROM admin_users LIMIT 1;
  END IF;
