...
-- Align vendors schema with extended payout test inserts (idempotent for existing DBs)
DO $$
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'address_line') THEN
    ALTER TABLE vendors ADD COLUMN address_line text DEFAULT '';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'city') THEN
    ALTER TABLE vendors ADD COLUMN city text DEFAULT '';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'state') THEN
    ALTER TABLE vendors ADD COLUMN state text DEFAULT '';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'zip_code') THEN
    ALTER TABLE vendors ADD COLUMN zip_code text DEFAULT '';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'country') THEN
    ALTER TABLE vendors ADD COLUMN country text DEFAULT '';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'bank_account_number') THEN
    ALTER TABLE vendors ADD COLUMN bank_account_number text DEFAULT '';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'bank_routing_number') THEN
    ALTER TABLE vendors ADD COLUMN bank_routing_number text DEFAULT '';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'commission_rate') THEN
    ALTER TABLE vendors ADD COLUMN commission_rate numeric(6,3) DEFAULT 0;
  END IF;
END $$;

UPDATE vendors v
SET
  phone = COALESCE(NULLIF(trim(v.phone), ''), NULLIF(trim(v.contact_phone), '')),
  address_line = COALESCE(NULLIF(trim(v.address_line), ''), NULLIF(trim(v.address->>'street'), '')),
  city = COALESCE(NULLIF(trim(v.city), ''), NULLIF(trim(v.address->>'city'), '')),
  state = COALESCE(NULLIF(trim(v.state), ''), NULLIF(trim(v.address->>'state'), '')),
  zip_code = COALESCE(NULLIF(trim(v.zip_code), ''), NULLIF(trim(v.address->>'zip'), '')),
  country = COALESCE(NULLIF(trim(v.country), ''), NULLIF(trim(v.address->>'country'), ''))
WHERE v.phone IS DISTINCT FROM COALESCE(NULLIF(trim(v.phone), ''), NULLIF(trim(v.contact_phone), ''))
   OR v.address_line IS DISTINCT FROM COALESCE(NULLIF(trim(v.address_line), ''), NULLIF(trim(v.address->>'street'), ''))
   OR v.city IS DISTINCT FROM COALESCE(NULLIF(trim(v.city), ''), NULLIF(trim(v.address->>'city'), ''))
   OR v.state IS DISTINCT FROM COALESCE(NULLIF(trim(v.state), ''), NULLIF(trim(v.address->>'state'), ''))
   OR v.zip_code IS DISTINCT FROM COALESCE(NULLIF(trim(v.zip_code), ''), NULLIF(trim(v.address->>'zip'), ''))
   OR v.country IS DISTINCT FROM COALESCE(NULLIF(trim(v.country), ''), NULLIF(trim(v.address->>'country'), ''));
...
BEGIN
  ...
  INSERT INTO vendors (
    user_id, business_name, contact_email, contact_phone, phone, address_line, address, city, state,
