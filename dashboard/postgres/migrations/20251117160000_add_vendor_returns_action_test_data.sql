...
BEGIN
  ...
  -- FK-safe order_id: use existing orders for each vendor, or insert a minimal seed order
  SELECT id INTO order1_id FROM orders WHERE vendor_id = vendor1_id ORDER BY created_at DESC NULLS LAST LIMIT 1 OFFSET 0;
  SELECT id INTO order2_id FROM orders WHERE vendor_id = vendor1_id ORDER BY created_at DESC NULLS LAST LIMIT 1 OFFSET 1;
  SELECT id INTO order3_id FROM orders WHERE vendor_id = vendor1_id ORDER BY created_at DESC NULLS LAST LIMIT 1 OFFSET 2;
  SELECT id INTO order4_id FROM orders WHERE vendor_id = vendor1_id ORDER BY created_at DESC NULLS LAST LIMIT 1 OFFSET 3;
  ...
  IF order1_id IS NULL THEN
    ...
    VALUES (
      vendor1_id, 'ORD-RET-SEED-V1', 'Return Seed', 'return.seed.v1@example.com',
      'Return', 'S',
      'delivered', 0, 'paid'
    )
    RETURNING id INTO order1_id;
  END IF;
  order2_id := COALESCE(order2_id, order1_id);
  order3_id := COALESCE(order3_id, order1_id);
  order4_id := COALESCE(order4_id, order1_id);

  SELECT id INTO new_vendor_order_id FROM orders WHERE vendor_id = new_vendor_id ORDER BY created_at DESC NULLS LAST LIMIT 1;
  IF new_vendor_order_id IS NULL THEN
    INSERT INTO orders (
      vendor_id, order_number, customer_name, customer_email,
      customer_first_name, customer_last_initial,
      status, total_amount, payment_status
    ...
    VALUES (
      new_vendor_id, 'ORD-RET-SEED-NV', 'Return Seed NV', 'return.seed.nv@example.com',
      'Return', 'N',
      'delivered', 0, 'paid'
    )
    RETURNING id INTO new_vendor_order_id;
  END IF;
  ...
  -- Pending (3)
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-001', 'pending', 'defective',
  ...
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-002', 'pending', 'wrong_item',
  ...
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-003', 'pending', 'damaged',
  ...
  -- Approved (2)
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-004', 'approved', 'wrong_size',
  ...
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-005', 'approved', 'not_as_described',
  ...
  -- Received (2)
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-006', 'received', 'defective',
  ...
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-007', 'received', 'changed_mind',
  ...
  -- Completed (2)
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-008', 'completed', 'defective',
  ...
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-009', 'completed', 'wrong_item',
  ...
  -- Rejected (1)
  (new_vendor_id, new_vendor_order_id, 'RET-20251117-V2-010', 'rejected', 'changed_mind',
