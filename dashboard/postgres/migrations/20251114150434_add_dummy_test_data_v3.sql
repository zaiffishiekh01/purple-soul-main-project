...
DO $$
DECLARE
  v_user_id uuid := '6bcb7173-959c-4b67-9b75-3ab5672e1b97';
  v_approved_user_id uuid := '3f6ea7a8-4ec6-4b78-9f97-7c4af40d7762';
  v_vendor_id uuid;
  v_approved_vendor_id uuid;
  v_product1_id uuid;
  v_product2_id uuid;
  v_product3_id uuid;
  v_product4_id uuid;
...
BEGIN
  ...
  -- Add a separate approved vendor for workflows that require approved status
  SELECT id INTO v_approved_vendor_id FROM vendors WHERE business_name = 'Approved Seed Vendor' LIMIT 1;

  IF v_approved_vendor_id IS NULL THEN
    INSERT INTO vendors (
      user_id,
      business_name,
      business_type,
      ...
      v_approved_user_id,
      'Approved Seed Vendor',
      'E-commerce',
      'approved.vendor@test.com',
      '+1-555-0456',
      jsonb_build_object(
        'street', '200 Approved Lane',
        'city', 'New York',
        'state', 'NY',
        'zip', '10002',
        'country', 'United States'
      ...
      'TAXAPPROVED001',
      'approved'
    ) RETURNING id INTO v_approved_vendor_id;
  END IF;
