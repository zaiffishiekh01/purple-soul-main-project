...
-- Re-runnable: same batch numbers / notes may already exist (e.g. duplicate-timestamp migration file)
UPDATE transactions
SET payout_request_id = NULL
WHERE payout_request_id IN (SELECT id FROM payout_requests WHERE notes LIKE '%TEST-WORKFLOW%');

DELETE FROM bank_transfer_logs
WHERE payout_request_id IN (SELECT id FROM payout_requests WHERE notes LIKE '%TEST-WORKFLOW%');

DELETE FROM payout_requests WHERE notes LIKE '%TEST-WORKFLOW%';
...
DECLARE
  ...
  payout_id UUID;
BEGIN
  -- Resolve vendors from DB (hardcoded UUIDs break on fresh seeds); match other test migrations
