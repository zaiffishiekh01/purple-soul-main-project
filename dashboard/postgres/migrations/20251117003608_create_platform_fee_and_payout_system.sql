...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'payout_requests' AND column_name = 'payout_details') THEN
    ALTER TABLE payout_requests ADD COLUMN payout_details jsonb;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'payout_requests' AND column_name = 'period_start') THEN
    ALTER TABLE payout_requests ADD COLUMN period_start timestamptz;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'payout_requests' AND column_name = 'period_end') THEN
    ALTER TABLE payout_requests ADD COLUMN period_end timestamptz;
  END IF;
